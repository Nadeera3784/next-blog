                    ...
                    <table className="w-full text-sm text-left rtl:text-right">
                        <thead className="text-xs text-gray-700 uppercase bg-gray-50">
                            <tr className="border-b">
                                <th scope="col" className="px-6 py-3 font-medium text-xs text-muted">
                                    Title
                                </th>
                                <th scope="col" className="px-6 py-3 font-medium text-xs text-muted">
                                    Views
                        ...
                        <tbody>
                            {posts.map((post) => (
                                <tr key={post._id} className="bg-white border-b  hover:bg-gray-50">
                                    <th scope="row" className="px-6 py-4">{post.title}</th>
                                    <td className="px-6 py-4">{post?.views}</td>
                                    <td className="px-6 py-4">
                                        <div className="inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 text-foreground">{post?.category.name}</div>
                                    </td>
                                    <td className="px-6 py-4"> {dayjs(post.createdAt).format("YYYY-MM-DD")}</td>
                                    <td className="px-6 py-4">
                                        <button
